...
async function loadKb() {
  ...
  try {
    ...
    kb.value =
      (page.list || []).find((item: any) => item.id === kbId.value) || null;
  } catch {
    kb.value = null;
  }
}
...
const publishedCount = () =>
  docs.value.filter((d) => d.parseStatus === 'PUBLISHED').length;
const reviewCount = () =>
  docs.value.filter((d) => d.parseStatus === 'REVIEW').length;
const failedCount = () =>
  docs.value.filter((d) => d.parseStatus === 'FAILED').length;
...
function openEvaluation() {
  router.push(knowledgeEvaluationRoute(kbId.value));
}
function openReview(documentId?: number) {
  router.push(knowledgeReviewRoute({ documentId, kbId: kbId.value }));
}
...
function viewVersions(documentId: number) {
  router.push(knowledgeVersionRoute(documentId));
}
function viewConflicts(documentId: number) {
  router.push(knowledgeConflictRoute(documentId));
}
function openAdvanced(doc: KnowledgeDocument) {
  router.push({
    path: '/kb/documents',
    query: { kbId: kbId.value, docId: doc.id },
  });
}
function editKb() {
  ...
  <Page auto-content-height>
    <div class="ws-head">
      <div>
        <Button
          type="link"
          class="!p-0"
          @click="router.push('/kb/knowledge-base')"
          >← 返回知识库</Button
        >
        ...
        <p class="ws-desc">
          ...
          {{ docs.length }} 份资料 · {{ publishedCount() }} 已发布 ·
          {{ reviewCount() }} 待审核 · {{ failedCount() }} 异常
        </p>
      </div>
      <Space>
    ...
    <Tabs v-model:active-key="activeTab">
      <Tabs.TabPane key="overview" tab="概览">
        <Row :gutter="16">
          <Col :span="6"
            ><Card><Statistic title="资料总数" :value="docs.length" /></Card
          ></Col>
          <Col :span="6"
            ><Card><Statistic title="已发布" :value="publishedCount()" /></Card
          ></Col>
          <Col :span="6"
            ><Card><Statistic title="待审核" :value="reviewCount()" /></Card
          ></Col>
          <Col :span="6"
            ><Card><Statistic title="异常" :value="failedCount()" /></Card
          ></Col>
        </Row>
        <Alert
          v-if="reviewCount() > 0"
          ...
          type="warning"
          show-icon
          :message="`有 ${reviewCount()} 份资料等待审核，发布后才参与正式问答。`"
        >
          <template #action
        ...
        <Alert
          ...
          type="error"
          show-icon
          :message="`有 ${failedCount()} 份资料处理失败，请查看失败原因并重试。`"
        >
          <template #action
            ><Button size="small" @click="goDocuments"
              >处理异常</Button
            ></template
          >
        </Alert>
        <Card title="业务流程" size="small" class="mt-4">
          <Space wrap>
            <Button type="primary" @click="goDocuments"
              >1. 上传 / 管理资料</Button
            >
      ...
      <Tabs.TabPane key="documents" tab="文档">
        ...
        >
          ...
          <Table.Column title="更新时间" width="170"
            ><template #default="{ record }">{{
              fmtTime(record.createTime)
            }}</template></Table.Column
          >
          <Table.Column title="操作" width="380" fixed="right">
            <template #default="{ record }">
              <Space>
                <a
                  v-if="record.parseStatus === 'REVIEW'"
                  @click="openReview(record.id)"
                  >去审核</a
                >
                <a @click="viewVersions(record.id!)">版本记录</a>
                <a
                  v-if="
                    record.parseStatus === 'REVIEW' ||
                    record.parseStatus === 'PUBLISHED'
                  "
                  @click="viewConflicts(record.id!)"
                  >冲突裁决</a
                >
      ...
      <Tabs.TabPane key="qa" tab="问答调试">
        <Card title="验证这个知识库能否正确回答" size="small">
          <p class="text-muted-foreground">
            使用与正式对话一致的检索、证据和回答链路；调试页额外展示 Query
            Planner 路由、证据、引用和模型执行信息。
          </p>
          <Space class="mt-3">
      ...
      <Tabs.TabPane key="quality" tab="质量">
        <Row :gutter="16">
          <Col :span="8"
            ><Card><Statistic title="待审核资料" :value="reviewCount()" /></Card
          ></Col>
          <Col :span="8"
            ><Card><Statistic title="处理异常" :value="failedCount()" /></Card
          ></Col>
          <Col :span="8"
            ><Card
              ><Statistic
                title="结构化元数据完整"
                :value="`${withMetadata()}/${docs.length}`" /></Card
          ></Col>
        </Row>
        <Card title="质量闭环" size="small" class="mt-4">
          <p class="text-muted-foreground">
            文档内容问题在“审核发布”处理；回答质量通过评测用例持续回归。运行链路用于定位异常，不等同于知识质量。
          </p>
          <Space class="mt-3">
