...
import { ElAvatar, ElButton, ElCard, ElEmpty } from 'element-plus';
...
<template>
  <ElCard class="min-h-[130px]" shadow="never">
    <template v-if="employee">
      <div class="flex items-start gap-8 px-5 py-4">
        <ElAvatar :size="64" :src="employee.avatar">
          ...
          <div class="text-muted-foreground mt-5 leading-8">
            ...
            <span v-if="showRegularDate">
              ...
              <b class="text-foreground font-bold">
                {{
                  formatHrmDate(
                    employee.regularTime ? +employee.regularTime : undefined,
                  )
                }}
              </b>
              转正
    ...
    <ElEmpty v-else description="当前账号未绑定员工档案" />
  </ElCard>
</template>
