...
import { ElCard, ElMessage, ElPagination } from 'element-plus';
...
function openAppealForm(id?: number) {
  if (id) {
    appealFormRef.value?.open(id);
  }
}

function openAppealHandleForm(assessmentId?: number, stageId?: number) {
  if (assessmentId && stageId) {
    appealHandleFormRef.value?.open(assessmentId, stageId);
  }
}
  ...
  <Page v-if="accessible">
    <ElCard shadow="never">
      ...
      <PerformanceTaskTable
        ...
        :list="list"
        :loading="loading"
        @appeal="openAppealForm"
        @appeal-handle="openAppealHandleForm"
        @detail="openDetail"
