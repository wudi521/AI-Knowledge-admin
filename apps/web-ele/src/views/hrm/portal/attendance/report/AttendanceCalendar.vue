...
/** 获取日期标签类型 */
function getDayTagType(detail?: HrmAttendanceStatisticsApi.DailyDetail) {
  if (isAbnormal(detail)) {
    return 'danger';
  }
  if (detail?.scheduled === false) {
...
<template>
  ...
  >
    <template #header>
      <div
        class="flex w-full items-center justify-between text-[15px] font-semibold"
      >
        <span>{{ formatDate(selectedMonth, 'YYYY年MM月') }}考勤明细</span>
        <div
          class="flex gap-4 text-xs font-normal text-[var(--el-text-color-secondary)]"
        >
          <span class="flex items-center gap-1">
            <i
              class="inline-block h-[7px] w-[7px] rounded-full bg-[var(--el-color-success)]"
            ></i>
            正常
          </span>
          <span class="flex items-center gap-1">
            <i
              class="inline-block h-[7px] w-[7px] rounded-full bg-[var(--el-color-danger)]"
            ></i>
            异常
          </span>
          <span class="flex items-center gap-1">
            <i
              class="inline-block h-[7px] w-[7px] rounded-full bg-[var(--el-color-info)]"
            ></i>
            休息
          </span>
        </div>
      ...
      >
        ...
        <template v-if="data.type === 'current-month'">
          <div
            ...
            class="mt-1 flex justify-between text-xs text-[var(--el-text-color-secondary)]"
          >
            ...
            <em
              class="not-italic text-[var(--el-color-success)]"
          ...
          <div
            v-if="
              getDailyDetail(data.day) &&
              !(getDailyDetail(data.day)?.clockList || []).length
            "
            class="mt-3 text-center text-xs text-[var(--el-text-color-placeholder)]"
          >
            暂无打卡
          </div>
        </template>
