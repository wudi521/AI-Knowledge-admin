...
onMounted(() => {
  if (!Number.isSafeInteger(candidateId) || candidateId <= 0) {
    ElMessage.warning('参数错误，招聘候选人不能为空！');
    close();
    return;
  }
  getCandidate();
  ...
  <Page auto-content-height :loading="loading">
    ...
    <ElCard class="mt-4">
      <ElTabs v-model="activeTab">
        <ElTabPane label="详细资料" name="detail">
          <Info :candidate="candidate" />
        </ElTabPane>
        <ElTabPane label="材料附件" name="file" lazy>
          <MaterialFiles :candidate="candidate" />
        </ElTabPane>
        <ElTabPane label="面试记录" name="interview" lazy>
          <InterviewList :interview-list="interviewList" />
        </ElTabPane>
        <ElTabPane label="操作日志" name="operateLog">
