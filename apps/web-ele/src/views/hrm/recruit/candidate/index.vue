...
function getRowDropDownActions(row: HrmRecruitCandidateApi.RecruitCandidate) {
  return getMoreActions(row).map((action) => ({
    label: action.label,
    auth: [] as string[],
    type: (action.command === 'delete' ? 'danger' : 'primary') as
      | 'danger'
      | 'primary',
    link: true,
    ...
    <Grid class="candidate-grid">
      <template #toolbar-actions>
        <div class="candidate-status-tabs">
          ...
          >
            <ElTabPane name="all" :label="`全部（${allStatusCount}）`" />
            <ElTabPane
...
.candidate-status-tabs :deep(.el-tabs__header) {
  margin-bottom: 0;
}

.candidate-status-tabs :deep(.el-tabs__nav-wrap::after) {
  display: none;
}
</style>
