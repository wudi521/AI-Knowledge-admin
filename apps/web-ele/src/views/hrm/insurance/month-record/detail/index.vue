...
async function init() {
  if (!Number.isSafeInteger(monthRecordId) || monthRecordId <= 0) {
    ElMessage.warning('参数错误，月度社保表不能为空！');
    close();
    return;
  }
  await Promise.all([getMonthRecord(), gridApi.query()]);
...
<template>
  <Page auto-content-height content-class="flex flex-col overflow-hidden">
    ...
    <div class="mb-4 shrink-0" v-loading="recordLoading">
      <ElCard>
        <ElDescriptions :column="3" border size="small">
          <ElDescriptionsItem label="参保人数">
            <ElButton
              link
              type="primary"
